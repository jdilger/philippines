...
def upload( manifest : str =None):
    ...
    result = subprocess.run(["earthengine", "upload", "image", "--manifest", manifest], capture_output=True)
    task_id = re.search(r"(Started upload task with ID:) ([\w{1:24}\d{1:24}]*)",str(result))
